...
( window.WebergencyCloud.Password = ( function()
{
    function encode( str )
    {
    ...
    return new Proxy( function Password(){}, 
    {
        construct( target, args )
        {
            console.log( args );

            let [ password, salt ] = args.map( i => typeof i === 'string' ? digest( 'SHA-256', i ) : i );

            class Password
            {
                async hash()
                {
                    return new Uint8Array( await pbkdf2_hmac( 'SHA-256', await password, await salt, 32, 100000 ));
                }

                async key()
                {
                    return scrypt( await password, await salt );
                }
            }
